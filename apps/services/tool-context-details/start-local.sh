...
# Start GetContextDetails service locally without SAM
# This script loads environment variables from env.json and starts the server

echo "🚀 Starting GetContextDetails service locally..."
echo "======================================"
...
if command -v jq &> /dev/null; then
    ...
    export $(jq -r '.GetContextDetailsFunction | to_entries | .[] | "\(.key)=\(.value)"' env.json)
else
    # Fallback to node
    echo "Loading environment variables from env.json using node..."
    eval $(node -e "
        const config = require('./env.json');
        const vars = config.GetContextDetailsFunction;
